...
3. Configuración:
   ...
   - **Root Directory:** déjalo **vacío** (raíz del repo), para que el build acceda a `packages/shared` y `server`.
   - **Runtime:** Node
   - **Build Command:**
     ```bash
     cd packages/shared && npm install && npm run build && cd ../../server && npm install && npx prisma generate && npx prisma db push && npm run build
     ```
   - **Start Command:** `cd server && npx prisma db push && node index.js`
