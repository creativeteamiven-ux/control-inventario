...
export default function AddMaintenanceModal({ open, onOpenChange }: AddMaintenanceModalProps) {
  ...
  useEffect(() => {
    if (open && !form.deviceId && devices.length > 0) {
      setForm((f) => ({ ...f, deviceId: devices[0]?.id || '' }));
    }
  }, [open, devices, form.deviceId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/api/maintenance', {
        ...
        ...(canViewCost() && form.cost && { cost: parseFloat(form.cost) }),
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={submitting}>
              {submitting ? 'Guardando...' : 'Agregar mantenimiento'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
